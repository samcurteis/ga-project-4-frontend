...
import AuthorPage from '../features/authorPage/AuthorPage.tsx'
import "../setupTests"
...
describe('favorite functionality', () => {
  ...
  it("increments favorite counter by one when user clicks on favorite button", async () => {
    server.use(
      rest.put(`${process.env.REACT_APP_BASE_URL}/api/authors/:id/`, (_req, res, ctx) => {
        return res(
          ctx.status(200),
          ctx.json({
            name: 'Poet',
    ...
    expect(updatedFavorites).toBeInTheDocument()
  })
})
